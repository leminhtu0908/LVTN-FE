...
const MemoryCreateDialog = (props) => {
  const handleCloseDialog = () => {
    props.closeCreateDialog(false);
  };
  const dispatch = useDispatch();
  const schemaValidation = Yup.object().shape({
    name: Yup.string().required("Vui lòng nhập tên bộ nhớ"),
  });
  const {
    control,
    handleSubmit,
  ...
  return (
    <>
      ...
      >
        {props.isEdit ? (
          <DialogTitle>{"Cập nhật bộ nhớ"}</DialogTitle>
        ) : (
          <DialogTitle>{"Thêm bộ nhớ"}</DialogTitle>
        )}
        <div className="p-4">
          <form onSubmit={handleSubmit(handleSumitCategory)}>
            <InputAdmin
              placeholder="Tên bộ nhớ"
              type="text"
              control={control}
              name="name"
            />
